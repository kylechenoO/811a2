...
% call function
[x1, x2, x3] = GaussSeidlMethod(x1, x2, x3, precision);
fprintf('The final value is: [x1=%.9f][x2=%.9f][x3=%.9f]\n', x1, x2, x3);

% function part
% the GaussSeidlMethod function input(x, precision), return(y)
function [x1, x2, x3] = GaussSeidlMethod(x1, x2, x3, precision)
...
function [x1, x2, x3] = fn(x1_before, x2_before, x3_before)
    ...
    x2 = -0.4 * x1 + 0.5 * x3_before + 0.2;
    x3 = - 0.5 * x1 - 0.1 * x2 + 0.15;
end
